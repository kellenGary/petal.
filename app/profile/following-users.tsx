...
export default function FollowingUsersScreen() {
    ...
    useEffect(() => {
        ...
        setHasMore(true);
        setLoading(true);
        fetchFollowing(0, true, debouncedQuery);
    }, [debouncedQuery, parsedUserId]);

    const fetchFollowing = useCallback(async (offset: number, isRefresh: boolean, query: string) => {
        ...
        try {
            const response = await followApi.getFollowing(parsedUserId, PAGE_SIZE, offset, query);
    ...
    const handleLoadMore = useCallback(() => {
        if (!loadingMore && hasMore) {
            fetchFollowing(users.length, false, debouncedQuery);
        }
    }, [loadingMore, hasMore, users.length, fetchFollowing, debouncedQuery]);

    const handleRefresh = useCallback(() => {
        fetchFollowing(0, true, debouncedQuery);
    }, [fetchFollowing, debouncedQuery]);
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
            ...
            >
                ...
                <ThemedText style={[styles.title, { color: colors.text }]}>Following</ThemedText>
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyContainer}>
                            ...
                            <ThemedText style={[styles.emptyText, { color: colors.text }]}>
                                {debouncedQuery ? "No results found" : "Not following anyone yet"}
                            </ThemedText>
                        </View>
                    }
                    ListFooterComponent={
